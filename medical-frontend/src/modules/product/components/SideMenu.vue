<script setup>
    import { ref } from 'vue';
    import useAuth from '../../auth/composables/useAuth';
    
    const { logoutUser } = useAuth()

    
    </script>
    
    <template>
        <aside class="aside">
            <nav class="nav">
                <ul class="nav__menu">
                    <li class="menu__item">
                        
                        <router-link :to="{ name: 'list-products' }" class="item__link">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M14.6998 22.7487H9.29977C4.35977 22.7487 2.25977 20.6387 2.25977 15.7087V11.2188C2.25977 10.8088 2.59977 10.4688 3.00977 10.4688C3.41977 10.4688 3.75977 10.8088 3.75977 11.2188V15.7087C3.75977 19.7987 5.20977 21.2487 9.29977 21.2487H14.6898C18.7798 21.2487 20.2298 19.7987 20.2298 15.7087V11.2188C20.2298 10.8088 20.5698 10.4688 20.9798 10.4688C21.3898 10.4688 21.7298 10.8088 21.7298 11.2188V15.7087C21.7398 20.6387 19.6298 22.7487 14.6998 22.7487Z" fill="#292D32"/>
                                <path d="M11.999 12.75C10.899 12.75 9.89902 12.32 9.18902 11.53C8.47902 10.74 8.14902 9.71 8.25902 8.61L8.92903 1.93C8.96903 1.55 9.28903 1.25 9.67903 1.25H14.349C14.739 1.25 15.059 1.54 15.099 1.93L15.769 8.61C15.879 9.71 15.549 10.74 14.839 11.53C14.099 12.32 13.099 12.75 11.999 12.75ZM10.349 2.75L9.74902 8.76C9.67902 9.43 9.87903 10.06 10.299 10.52C11.149 11.46 12.849 11.46 13.699 10.52C14.119 10.05 14.319 9.42 14.249 8.76L13.649 2.75H10.349Z" fill="#292D32"/>
                                <path d="M18.31 12.75C16.28 12.75 14.47 11.11 14.26 9.09L13.56 2.08C13.54 1.87 13.61 1.66 13.75 1.5C13.89 1.34 14.09 1.25 14.31 1.25H17.36C20.3 1.25 21.67 2.48 22.08 5.5L22.36 8.28C22.48 9.46 22.12 10.58 21.35 11.43C20.58 12.28 19.5 12.75 18.31 12.75ZM15.14 2.75L15.76 8.94C15.89 10.19 17.05 11.25 18.31 11.25C19.07 11.25 19.75 10.96 20.24 10.43C20.72 9.9 20.94 9.19 20.87 8.43L20.59 5.68C20.28 3.42 19.55 2.75 17.36 2.75H15.14V2.75Z" fill="#292D32"/>
                                <path d="M5.63904 12.75C4.44904 12.75 3.36904 12.28 2.59904 11.43C1.82904 10.58 1.46904 9.46 1.58904 8.28L1.85904 5.53C2.27904 2.48 3.64904 1.25 6.58905 1.25H9.63904C9.84904 1.25 10.049 1.34 10.199 1.5C10.349 1.66 10.409 1.87 10.389 2.08L9.68904 9.09C9.47904 11.11 7.66904 12.75 5.63904 12.75ZM6.58905 2.75C4.39904 2.75 3.66904 3.41 3.34904 5.7L3.07904 8.43C2.99904 9.19 3.22904 9.9 3.70904 10.43C4.18904 10.96 4.86904 11.25 5.63904 11.25C6.89904 11.25 8.06904 10.19 8.18904 8.94L8.80905 2.75H6.58905V2.75Z" fill="#292D32"/>
                                <path d="M14.5 22.75H9.5C9.09 22.75 8.75 22.41 8.75 22V19.5C8.75 17.4 9.9 16.25 12 16.25C14.1 16.25 15.25 17.4 15.25 19.5V22C15.25 22.41 14.91 22.75 14.5 22.75ZM10.25 21.25H13.75V19.5C13.75 18.24 13.26 17.75 12 17.75C10.74 17.75 10.25 18.24 10.25 19.5V21.25Z" fill="#292D32"/>
                            </svg> 
                            Productos
                        </router-link>
                    </li>
                    <li class="menu__item">
                        <router-link :to="{ name: 'new-product' }" class="item__link"> 
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M14.7205 22.751H9.33056C8.69056 22.751 8.13054 22.711 7.62054 22.641C7.21054 22.581 6.92052 22.201 6.98052 21.791C7.04052 21.381 7.41056 21.081 7.83056 21.151C8.27056 21.211 8.76055 21.241 9.32055 21.241H14.7106C18.8006 21.241 20.2505 19.7909 20.2505 15.7009V11.2109C20.2505 10.8009 20.5905 10.4609 21.0005 10.4609C21.4105 10.4609 21.7505 10.8009 21.7505 11.2109V15.7009C21.7605 20.6409 19.6505 22.751 14.7205 22.751Z" fill="#292D32"/>
                                <path d="M3.03906 16.2688C2.62906 16.2688 2.28906 15.9288 2.28906 15.5188V11.2188C2.28906 10.8088 2.62906 10.4688 3.03906 10.4688C3.44906 10.4688 3.78906 10.8088 3.78906 11.2188V15.5188C3.78906 15.9288 3.44906 16.2688 3.03906 16.2688Z" fill="#292D32"/>
                                <path d="M12.0303 12.75C10.9303 12.75 9.93027 12.32 9.22027 11.53C8.51027 10.74 8.18028 9.70999 8.29028 8.60999L8.96032 1.93005C9.00032 1.55005 9.32032 1.25 9.71032 1.25H14.3803C14.7703 1.25 15.0903 1.54005 15.1303 1.93005L15.8003 8.60999C15.9103 9.70999 15.5803 10.74 14.8703 11.53C14.1303 12.32 13.1303 12.75 12.0303 12.75ZM10.3703 2.75L9.77032 8.76001C9.70032 9.43001 9.9003 10.06 10.3203 10.52C11.1703 11.46 12.8703 11.46 13.7203 10.52C14.1403 10.05 14.3403 9.42001 14.2703 8.76001L13.6703 2.75H10.3703Z" fill="#292D32"/>
                                <path d="M18.3295 12.75C16.2995 12.75 14.4895 11.11 14.2795 9.08997L13.5795 2.07996C13.5595 1.86996 13.6295 1.66 13.7695 1.5C13.9095 1.34 14.1095 1.25 14.3295 1.25H17.3795C20.3195 1.25 21.6895 2.48 22.0995 5.5L22.3795 8.28003C22.4995 9.46003 22.1395 10.5801 21.3695 11.4301C20.5995 12.2801 19.5195 12.75 18.3295 12.75ZM15.1595 2.75L15.7795 8.93994C15.9095 10.1899 17.0795 11.25 18.3295 11.25C19.0895 11.25 19.7695 10.9601 20.2595 10.4301C20.7395 9.90005 20.9595 9.19005 20.8895 8.43005L20.6095 5.68005C20.2995 3.42005 19.5695 2.75 17.3795 2.75H15.1595Z" fill="#292D32"/>
                                <path d="M5.67028 12.75C4.48028 12.75 3.4003 12.2801 2.6303 11.4301C1.8603 10.5801 1.50029 9.46003 1.62029 8.28003L1.90032 5.53003C2.32032 2.48003 3.6903 1.25 6.6303 1.25H9.68029C9.89029 1.25 10.0903 1.34 10.2403 1.5C10.3803 1.66 10.4503 1.86996 10.4303 2.07996L9.73028 9.08997C9.51028 11.11 7.69028 12.75 5.67028 12.75ZM6.62029 2.75C4.43029 2.75 3.7003 3.40995 3.3803 5.69995L3.11028 8.42004C3.03028 9.18004 3.26029 9.89004 3.74029 10.42C4.22029 10.95 4.91028 11.24 5.67028 11.24C6.93028 11.24 8.10027 10.1801 8.22027 8.93005L8.84033 2.73999H6.62029V2.75Z" fill="#292D32"/>
                                <path d="M5 23.7505C3.6 23.7505 2.29001 23.1404 1.39001 22.0704C1.38001 22.0604 1.34 22.0205 1.31 21.9705C1.16 21.7905 1.02999 21.6105 0.929993 21.4205C0.489993 20.7105 0.25 19.8705 0.25 18.9905C0.25 17.5305 0.900029 16.1805 2.03003 15.2805C2.23003 15.1205 2.45 14.9705 2.69 14.8405C3.38 14.4505 4.19 14.2305 5 14.2305C6.16 14.2305 7.23003 14.6305 8.09003 15.3805C8.20003 15.4605 8.34998 15.6105 8.47998 15.7505C9.28998 16.6405 9.73999 17.7805 9.73999 18.9705C9.73999 19.8405 9.49999 20.6905 9.04999 21.4205C8.82999 21.8005 8.56001 22.1305 8.26001 22.3905C8.20001 22.4505 8.14001 22.5105 8.07001 22.5605C7.28001 23.3305 6.17 23.7505 5 23.7505ZM2.47998 21.0405C2.50998 21.0705 2.55998 21.1204 2.59998 21.1904C3.14998 21.8404 4.05 22.2605 5 22.2605C5.79 22.2605 6.55 21.9704 7.13 21.4504C7.16 21.4104 7.2 21.3705 7.25 21.3405C7.45 21.1705 7.62003 20.9504 7.78003 20.6904C8.09003 20.1804 8.26001 19.6105 8.26001 19.0105C8.26001 18.2005 7.95001 17.4105 7.39001 16.8005C7.32001 16.7205 7.24997 16.6404 7.15997 16.5804C6.07997 15.6504 4.59998 15.5105 3.41998 16.1805C3.25998 16.2705 3.11999 16.3605 2.98999 16.4705C2.20999 17.0905 1.76001 18.0105 1.76001 19.0105C1.76001 19.6005 1.91998 20.1805 2.22998 20.6805C2.30998 20.8205 2.38998 20.9305 2.47998 21.0305C2.46998 21.0305 2.47998 21.0305 2.47998 21.0405Z" fill="#292D32"/>
                                <path d="M6.49976 19.7305H3.50977C3.09977 19.7305 2.75977 19.3905 2.75977 18.9805C2.75977 18.5705 3.09977 18.2305 3.50977 18.2305H6.49976C6.90976 18.2305 7.24976 18.5705 7.24976 18.9805C7.24976 19.3905 6.90976 19.7305 6.49976 19.7305Z" fill="#292D32"/>
                                <path d="M5 21.2595C4.59 21.2595 4.25 20.9195 4.25 20.5095V17.5195C4.25 17.1095 4.59 16.7695 5 16.7695C5.41 16.7695 5.75 17.1095 5.75 17.5195V20.5095C5.75 20.9295 5.41 21.2595 5 21.2595Z" fill="#292D32"/>
                            </svg>
                            Agregar Productos
                        </router-link>
                    </li>
                </ul>
            </nav>
            <div class="separator">
            </div>
            <div 
                class="logout"
                @click="logoutUser"
            >
                <img src="@/assets/icons/logout.svg" alt="" class="logout__img">
                <span>Cerrar Sesión</span>
            </div>
        </aside>
    </template>
    
    <style scoped>
    .aside{
        display: flex;
        flex-direction: column;
        align-items: center;
        justify-content: center;
        gap: 3.2rem;
        width: 31.4rem;
        height: 34rem;
        background-color: var(--background-component-color);
        box-shadow: var(--shadow);
        border-radius: 10px;
        font-weight: 500;
    }
    .nav__menu {
        display: flex;
        flex-direction: column;
        gap: 2.4rem;
    }
    .menu__item {
        list-style: none;
    }
    .item__link {
        display: flex;
        width: 20rem;
        align-items: center;
        gap: 16px;
        font-size: 1.6rem;
        text-decoration: none;
        color: var(--text-inactive);
        padding: 10px;
    }
    .item__link path{
        fill: var(--text-inactive);
    }
    .router-link-active {
        color: var(--color-primary);
        background-color: #F1FAFF;
        border-radius: 8px;
        transition: all .5s ease-in-out;
    
    }
    .router-link-active path{
        fill: var(--color-primary);
    }
    
    .separator {
        width: 204px;
        height: 1px;
        background-color: var(--color-acentuacion);
    }
    .logout{
        display: flex;
        gap: .8rem;
        align-items: center;
        cursor: pointer;
        padding: 10px;
        border-radius: 8px;
    }
    .logout span {
        font-size: 1.6rem;
        color: var(--text-inactive);
    }
    .logout:hover {
        background-color: #FFDCDC;
    }
    .logout:hover span {
        color: var(--color-secundary);
    }
    </style>